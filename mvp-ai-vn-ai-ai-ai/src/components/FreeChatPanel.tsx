    ...
    <section className="glass-panel pointer-events-auto w-full max-w-4xl rounded-lg p-3">
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-[220px_1fr]">
        ...
        <div className="flex min-w-0 flex-col gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                {suggestions.map((suggestion) => (
                  <button
                    ...
                    className="rounded-md border border-amber-100/15 bg-amber-200/10 px-2.5 py-1 text-left text-xs leading-5 text-amber-50 transition hover:border-amber-200/45 hover:bg-amber-200/18 disabled:cursor-not-allowed disabled:opacity-55"
                  >
                    {suggestion.text}
                  </button>
                ))}
          ...
          {loading && selectedNpc && (
            <div
              className="inline-flex w-fit items-center rounded-md border border-amber-100/20 bg-black/30 px-3 py-2 text-sm text-amber-100"
              aria-live="polite"
            >
              <span>{selectedNpc.name}正在思考</span>
              <span className="thinking-dots ml-1 inline-flex w-6 items-center" aria-hidden="true">
                <span>.</span>
                <span>.</span>
                <span>.</span>
              </span>
