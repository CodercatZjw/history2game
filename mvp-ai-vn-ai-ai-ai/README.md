...
当前版本支持两种资源：剧情场景仍可使用网络 URL 或本地文件；NPC 立绘和结局 CG 已放在 `public/assets/`，通过 `/assets/...` 从本项目服务。你可以继续使用网络 URL，也可以把图片放入 `public/assets/backgrounds/` 或 `src/assets/backgrounds/`，再在 `assets.ts` 中替换对应字段。
